...
void check_onegin_for_openning(Onegin_type* onegin) // Checks the file with original Onegin for openning 
{
    ...
    {   
        ...
        onegin->error_code = ERR_OPEN_FILE_ONEGIN_TEXT;
        onegin->onegin_original = checked_file_onegin;
...
void check_sorted_left_file_for_openning(Onegin_type* onegin) // Checks the file with sorted Onegin text for openning 
{   
    ...
    if(checked_left_file_sorted_onegin == nullptr)
    {
        ...
        onegin->error_code = ERR_OPEN_FILE_SORTED_LEFT_ONEGIN_TEXT;
        onegin->onegin_left_sorted = checked_left_file_sorted_onegin;
        onegin_dump(onegin, FUNC_NAME, FUNC_LINE);
        onegin_dtor(onegin);
    }
    else
...
void check_sorted_right_file_for_openning(Onegin_type* onegin) // Checks the file with sorted Onegin text for openning 
{   
    ...
    if(checked_right_file_sorted_onegin == nullptr)
    {
        ...
        onegin->error_code = ERR_OPEN_FILE_SORTED_RIGHT_ONEGIN_TEXT;
        onegin->onegin_right_sorted = checked_right_file_sorted_onegin;
        onegin_dump(onegin, FUNC_NAME, FUNC_LINE);
        onegin_dtor(onegin);
    }
    else
...
const char* enum_to_string(size_t error_code) // (OK) Converts an enum's int value to the enum's string value 
{
    switch(error_code)
    {
        case ERR_OPEN_FILE_ONEGIN_TEXT:
            return "ERR_OPEN_FILE_ONEGIN_TEXT";
            break;
        case ERR_OPEN_FILE_SORTED_RIGHT_ONEGIN_TEXT:
            return "ERR_OPEN_FILE_SORTED_RIGHT_ONEGIN_TEXT";
            break;
        case ERR_OPEN_FILE_SORTED_LEFT_ONEGIN_TEXT:
            return "ERR_OPEN_FILE_SORTED_LEFT_ONEGIN_TEXT";
            break;
        case ERR_EMPTY_FILE_ONEGIN_TEXT:
            return "ERR_EMPTY_FILE_ONEGIN_TEXT";
            break;
        case ERR_CLOSE_FILE_ONEGIN_TEXT:
            return "ERR_CLOSE_FILE_ONEGIN_TEXT";
            break;
        case ERR_CLOSE_FILE_SORTED_RIGHT_ONEGIN_TEXT:
            return "ERR_CLOSE_FILE_SORTED_RIGHT_ONEGIN_TEXT";
            break;
        case ERR_CLOSE_FILE_SORTED_LEFT_ONEGIN_TEXT:
            return "ERR_CLOSE_FILE_SORTED_LEFT_ONEGIN_TEXT";
            break;
        case ERR_NULLPTR_BUFFER:
            return "ERR_NULLPTR_BUFFER";
            break;
        case ERR_NULLPTR_STRINGS:
            return "ERR_NULLPTR_STRINGS";
            break;
        case ERR_NULLPTR_STRING:
            return "ERR_NULLPTR_STRING";
            break;
        default:
            return "STACK IS OK"; 
            break;
...
void onegin_dump(Onegin_type* onegin, const char* FUNCTION_NAME, size_t FUNCTION_LINE) // (OK) Outputs all information about the onegin stack into the LOG_FILE.txt
{
    ...
    fprintf(logfile, "Error code = %ld (%s)\n", onegin->error_code, enum_to_string(onegin->error_code));
    ...
    fprintf(logfile, "Total number of lines: %ld\n\n", onegin->number_of_lines);
    fprintf(logfile, "--------------------END OF LOG_SESSION--------------------------\n\n");

    fclose(logfile);
}

void get_num_of_chars_in_file(Onegin_type* onegin) // (OK) Returns the sizeof(number of chars) onegin 
{   
    rewind(onegin->onegin_original); // Returns the pointer inside the file to the initial position

    fseek(onegin->onegin_original, 0, SEEK_END); // Puts the pointer to the end of file 
    onegin->size_of_onegin = ftell(onegin->onegin_original); // Returns the total number of bytes (1 char = 1 byte) -> number of chars WITH \R and without EOF
    if(onegin->size_of_onegin == 0)
    {
...
void onegin_dtor(Onegin_type* onegin) // (OK) Frees all pointers and deletes 
{                                     // all data about onegin struct
    ...
    if(fclose(onegin->onegin_original))
    {   
        onegin->error_code = ERR_CLOSE_FILE_ONEGIN_TEXT;
        onegin_dump(onegin, FUNC_NAME, FUNC_LINE);
        onegin_dtor(onegin);
    }
    if(fclose(onegin->onegin_left_sorted))
    {
        onegin->error_code = ERR_CLOSE_FILE_SORTED_LEFT_ONEGIN_TEXT;
        onegin_dump(onegin, FUNC_NAME, FUNC_LINE);
        onegin_dtor(onegin);
    }
    if(fclose(onegin->onegin_right_sorted))
    {
        onegin->error_code = ERR_CLOSE_FILE_SORTED_RIGHT_ONEGIN_TEXT;
        onegin_dump(onegin, FUNC_NAME, FUNC_LINE);
        onegin_dtor(onegin);
    }
}
...
void read_onegin_into_buf(Onegin_type* onegin) // (OK) Reads onegin text into the buffer
{   
    rewind(onegin->onegin_original); // Returns the pointer inside the file to the initial position
    onegin->size_of_onegin = fread(onegin->buffer_of_chars, sizeof(char), onegin->size_of_onegin, onegin->onegin_original);
    rewind(onegin->onegin_original); // Returns the pointer inside the file to the initial position
}
...
void sort_strings(Onegin_type* onegin) // (OK) Sorts the array of strings
{   
    qsort(onegin->strings, onegin->number_of_lines, sizeof(char*), comparator_left_to_right);
    print_strings_to_file(onegin, onegin->onegin_left_sorted);

    qsort(onegin->strings, onegin->number_of_lines, sizeof(char*), comparator_right_to_left);
    print_strings_to_file(onegin, onegin->onegin_right_sorted);  
}
